...
if v.PHASE == 2 % no delay in phase 1
    ...
    if v.NP(75) > 0  % IkBd inducible txn delay
        if t > v.NP(75)
            delayed_nfkbn_d = ...
                interp1(DELAY_NFKB{1}(1:DELAY_NFKB{3}-1),...
...
%TNFR part
flux_pd_m_tnf      = v.IP(1)    * TNF ; % pd_m_tnf 45' half life of exogenous TNF
...
flux_a_C1          = v.IP(9)    * C1_off;% C1_off --> C1
flux_C1_off        = v.IP(10)   * C1;% C1     --> C1_off
flux_C1_A20        = v.IP(11)   * C1 * a20;% C1     --> C1_off (A20 Mediated)
flux_d_C1          = v.IP(12)   * C1; %UPDATE % C1     --> TNFR + TTR
flux_i_C1_off      = v.IP(13)   * C1_off; % C1_off internalization
...
flux_C1tnf_A20     = v.IP(23)   * C1tnf * a20;  % C1tnf     --> C1tnf_off (A20 Mediated)
...
delta_IkBdt     = delta_IkBdt - flux_rd_d;
...
delta_IkBd      = delta_IkBd - flux_ph_c_d;
...
delta(37,1)     = delta_a20 ;
delta(38,1)     = delta_a20t ;
end 